...
def hearding_update(datasets, buffer, feature_extractor, device):
    ...
    for cls in range(buffer.total_classes):
        ...
        cls_selected_images, cls_selected_labels = construct_examplar(copy.copy(datasets), cls_images, cls_labels,
...
def foster_update(fixed_memory, datasets, buffer, feature_extractor, device, per_classes, start_cls_idx, end_cls_idx, test_trsfs):
    if fixed_memory:
        herding_update_unified(datasets, buffer, feature_extractor, device, per_classes, start_cls_idx, end_cls_idx,
                               test_trsfs)
    else:
        reduce_buffer_examplar(buffer, per_classes, start_cls_idx)
        herding_update_unified(datasets, buffer, feature_extractor, device, per_classes, start_cls_idx, end_cls_idx,
                               test_trsfs)


def herding_update_unified(datasets, buffer, feature_extractor, device, per_classes, start_cls_idx, end_cls_idx,
                           test_trsfs):
    """
    ...
    for cls in range(start_cls_idx, end_cls_idx):
        ...
        cls_selected_images, cls_selected_labels = construct_examplar_foster(copy.copy(datasets), cls_images,
                                                                             cls_labels,
                                                                             feature_extractor, per_classes, device,
                                                                             test_trsfs)
        selected_images.extend(cls_selected_images)
