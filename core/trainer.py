        ...
        for task_idx in range(self.task_num):
            ...
            if task_idx == 0:
                self.optimizer = optim.SGD(
                    ...
                    momentum=0.9,  # 动量
                    lr=args["init_lr"],  # 学习率
                    weight_decay=args["init_weight_decay"],  # 权重衰减
                )
            ...
            if self.buffer.strategy == 'herding':
                ...
                foster_update(self.model._fixed_memory, self.train_loader.get_loader(task_idx).dataset, self.buffer,
                              self.model.backbone, self.device, self.samples_per_class, start_cls_idx,
                              end_cls_idx, test_trfms)
